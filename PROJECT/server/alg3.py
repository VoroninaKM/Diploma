...
def maxFitness(individual, X_train, X_test, y_train, y_test):
    gb = GradientBoostingClassifier(n_estimators=individual[0], criterion=individual[1], 
...
def mutate(individual, muta):
    if muta == '1':
        ...
        if i == 1:
            ...
            individual[1] = random.choice(['friedman_mse', 'squared_error'])
        elif i == 3:
            individual[2] = random.choice([random.randint(2, 10), None])
        else:
            individual[3] = random.choice([2, 3, 4, 5])
...
async def GA(socket, sel='1', mate='1', PC=0.9, muta='1', PM=0.1, sizeP=5, sizeI=10):
    ...
    toolbox.register("criterion", random.choice, ['friedman_mse', 'squared_error'])
    toolbox.register("max_depth", random.choice, [random.randint(2, 10), None])
    toolbox.register("min_samples_split", random.choice, [2, 3, 4, 5])
