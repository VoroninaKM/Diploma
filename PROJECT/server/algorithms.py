...
async def eaSimple(population, toolbox, cxpb, mutpb, ngen, stats=None,
             halloffame=None, verbose=__debug__, socket=None, code=1, model=None, X_train=None, X_test=None, y_train=None, y_test=None):
    ...
    for ind, fit in zip(invalid_ind, fitnesses):
        ...
        await socket.send(f'0|{ind}|{fit}|{code}')
    ...
    if verbose:
        ...
        await socket.send(f"stats|{record['max']}|{record['avg']}|{record['std']}")
        await asyncio.sleep(2)
    ...
    for gen in range(1, ngen + 1):
        ...
        for ind, fit in zip(invalid_ind, fitnesses):
            ind.fitness.values = fit
            await socket.send(f'{gen}|{ind}|{fit}|{code}')
            await asyncio.sleep(2)
        ...
        plt.title('Зависимость макс/средней приспособленности от поколения')
        plt.savefig(f'gr{gen}.png')

        #print("Лучший индивидуум = ", hof.items[0])
        if code==1:
            lr = model(penalty=halloffame.items[0][0], C=halloffame.items[0][1], max_iter=halloffame.items[0][2],  solver='saga', l1_ratio=0.4)
            ...
            rf = model(n_estimators=halloffame.items[0][0], criterion=halloffame.items[0][1], 
                                max_depth=halloffame.items[0][2], min_samples_split=halloffame.items[0][3])
            ...
            gb = model(n_estimators=halloffame.items[0][0], criterion=halloffame.items[0][1], 
                        max_depth=halloffame.items[0][2], min_samples_split=halloffame.items[0][3])
        ...
        await socket.send(f'report|{bac}|{prec}|{rec}|{f1}|{gen}')
        await asyncio.sleep(2)

    await socket.send(f'{ngen+2}| | |{code}')
